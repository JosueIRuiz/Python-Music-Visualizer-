...
class VisualizerHub:
    def __init__(self, root):
        ...
        self.add_btn("Main Visualizer (Pitch)", os.path.join("live_visualizers", "main_visualizer.py"))
        self.add_btn("Average Frequency (Bar No Pitch)", os.path.join("live_visualizers", "average_frequency.py"))
        self.add_btn("Color Frequency (Bar Pitch)", os.path.join("live_visualizers", "color_frequency.py"))
        self.add_btn("Spectrogram", os.path.join("live_visualizers", "spectogram.py"))
        self.add_btn("Live Waveform", os.path.join("live_visualizers", "wave_visualizer.py"))

        self.section("FILE TOOLS (Auto-Export to MP4)", "#fab387")
        self.add_btn("Streaming Playback Controller", "file_controller.py", True)
        self.add_btn("Circular Visualizer (Pitch)", os.path.join("file_visualizers", "file_circle_pitch.py"), True)
        self.add_btn("Circular Visualizer (No Pitch)", os.path.join("file_visualizers", "file_circle_wave.py"), True)
        self.add_btn("Bar Graph (Pitch)", os.path.join("file_visualizers", "file_bar_pitch.py"), True)
        self.add_btn("Bar Graph (No Pitch)", os.path.join("file_visualizers", "file_bar_wave.py"), True)
        self.add_btn("Classic Waveform (File)", os.path.join("file_visualizers", "file_classic_wave.py"), True)
    ...
    def launch(self, script, is_file_tool):
        ...
        self.status_var.set(f"Running {os.path.basename(script)}...")
        self.progress.start()
