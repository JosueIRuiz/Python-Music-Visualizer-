...
while running:
    ...
    if rms >= 0.005:
        ...
        if np.sum(raw_spectrum) > 0:
            avg_freq = np.sum(freqs * raw_spectrum) / np.sum(raw_spectrum)
    ...
    freq_to_display = f"Average Frequency = {round(avg_freq, 2)} Hz" if avg_freq > 0 else "Average Frequency = 0 Hz"
    ...
    txt = font2.render(freq_to_display, True, (255, 255, 255))
    screen.blit(txt, txt.get_rect(center=(250, 50)))
